...
//   int dataSet = atol(argv[2]);
  ...
  /// setup plots
  plot pt;
  pt.setupCanvas(1,1,300,300); //设置canvas上图片的行数，列数
...
//   pt.draw2DFrames(748, 756);
  ...
  for(int fm1=0; fm1<807; fm1+=pt.nCol*pt.nRow){
    ...
    pt.draw2D();
    pt.c->SaveAs(TString::Format("gif1107/gif_%d_%d_%d.png", pedeSet,dataSet,fm1));
   }
